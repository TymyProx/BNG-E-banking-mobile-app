...
export default function AddBeneficiaryForm({ onSuccess }: AddBeneficiaryFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Succès",
        `${formData.firstName} ${formData.lastName} a été ajouté(e) à vos bénéficiaires avec succès.`,
        [
          {
            text: "OK",
            onPress: () => {
      ...
      Alert.alert("Erreur", "Une erreur est survenue lors de l'ajout du bénéficiaire.")
    } finally {
      setIsLoading(false)
    }
  }
